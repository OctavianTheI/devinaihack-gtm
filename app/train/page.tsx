        ...
        <div className={styles.workspaceGrid}>
            ...
            <div className={styles.callBody}>
              ...
              {!call ? (
                <div className={styles.startArea}>
                  ...
                  <label className={styles.consent}><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />I’ll use fictional details. I understand microphone audio may be processed by my browser’s speech service, and the transcript is sent to the configured AI provider for replies and grading.</label>
                  ...
                  <p className={styles.note}>For microphone practice, use Chrome or Edge on HTTPS or localhost. Headphones help avoid feedback.</p>
                </div>
              ) : (
                <>
          ...
          <aside className={styles.sidebar}>
            <section className={styles.settings} aria-labelledby="setup-title"><p className={styles.eyebrow}>Set the scene</p><h2 id="setup-title">Your call, your pitch.</h2>
              ...
              <label className={styles.audioToggle}><input type="checkbox" checked={coach.audioEnabled} onChange={(event) => coach.setAudioEnabled(event.target.checked)} />Speak prospect replies</label><p className={styles.note}>{config?.voice === "elevenlabs" ? "ElevenLabs Flash voice with browser-voice fallback." : "Browser voice. Configure ElevenLabs on the server for Flash voice."} Typed practice also works with audio off.</p>
            </section>
